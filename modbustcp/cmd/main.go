...
func main() {
	...
	server, err = modbus.NewServer(&serverConfig, rh)
	if err != nil {
...
func (eh *reqHandler) HandleCoils(req *modbus.CoilsRequest) (res []bool, err error) {
...
func (eh *reqHandler) HandleDiscreteInputs(req *modbus.DiscreteInputsRequest) (res []bool, err error) {
...
func (eh *reqHandler) HandleHoldingRegisters(req *modbus.HoldingRegistersRequest) (res []uint16, err error) {
	var regAddr uint16
...
func (eh *reqHandler) HandleInputRegisters(req *modbus.InputRegistersRequest) (res []uint16, err error) {
	var unixTs_s uint32
	...
	for regAddr := req.Addr; regAddr < req.Addr+req.Quantity; regAddr++ {
		...
		case 302:
			// returh the 16 most significant bits of the number
			res = append(res, uint16((math.Float32bits(3.1617)>>16)&0xffff))

		case 303:
			// returh the 16 least significant bits of the number
			res = append(res, uint16((math.Float32bits(3.1617))&0xffff))
